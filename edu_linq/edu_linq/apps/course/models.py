...
class Course(BaseModel):
    ...
    def lesson_list(self):
        """获取当前课程的前几节课时用于展示"""
        lesson_list = CourseLesson.objects.filter(is_show=True, is_del=False, course_id=self.id).all()
    ...
    def active_list(self):
        active_list = self.activeprices.filter(is_show=True, is_del=False,
    ...
    def real_expire_price(self, expire_id=0):
        ...
        :return:
        """
        expire_item = self.course_expire.filter(is_show=True, is_del=False, pk=expire_id)
        expire_price = expire_item[0].price
        ...
        if len(active_list) > 0:
            ...
            # 获取活动的优惠公式
            sale = active.discount.sale
            expire_price = float(expire_price)
            if expire_price >= condition:
                # 判断当前课程满足哪一种优惠条件
                if sale == "":
                    # 限时免费
                    expire_price = 0
                    ...
                    # 限时折扣
                    expire_price = expire_price * float(sale[1:])
                elif sale[0] == "-":
                    # x限时减免
                    expire_price = expire_price - float(sale[1:])
                    ...
                    for item in split:
                        ...
                        # 门槛价  优惠价
                        condition_price, condition_sale = sale_item.split("-")
                        if expire_price >= float(condition_price):
                            sale_list.append(float(condition_sale))

                    if len(sale_list) > 0:
                        # 课程原价减去当前满足条件中最高的优惠价格
                        expire_price = expire_price - max(sale_list)
        return expire_price
    ...
    def expire_text(self):
        """获取当前课程的有效期"""
        expires = self.course_expire.filter(is_show=True, is_del=False)
